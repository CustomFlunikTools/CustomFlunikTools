...
//							bottom: 80
						...
						logger: function (infolineHeader, infolineUnits, infolineSkipped, infolinemessage) {
							if (infolineSkipped != "") {
								console.log(infolineHeader+infolineUnits+" - Skipped: "+infolineSkipped+" - Upg: "+infolinemessage);
							} else {
								console.log(infolineHeader+infolineUnits+" - Upg: "+infolinemessage);
							}
						},
						...
						autoUpgrade: function () {
							...
							for (var nCity in ClientLib.Data.MainData.GetInstance().get_Cities().get_AllCities().d) {
								...
								for (var nUnit in offenceUnits.d) {
									...
									if (unitCanUpgrade) {
										if (unitlvl < lowestupgoffencelevel) {
								...
								if (currentcrypct>80) {
									...
									if (CC != null) {
										...
										if (CC.get_CurrentLevel() == lowestoffencelevel) {
											if (CC.CanUpgrade()) {
												console.log(infolineHeader+infolineUnits+" - Crystal is full - Upgrading CC since offencelevel is maximum");
												CC.Upgrade();
												continue;
											} else {
												var tryDHQ=true;
									...
									if (DHQ != null && tryDHQ) {
										if (DHQ.get_CurrentLevel() == lowestdefencelevel && DHQ.CanUpgrade()) {
											console.log(infolineHeader+infolineUnits+" - Crystal is full - Upgrading DHQ since defencelevel is maximum");
											DHQ.Upgrade();
											continue;
										}
									};
...
//this.Cache[ClientLib.Base.EResourceType.Gold][cname] = this.getPrioList(city, [ClientLib.Base.ETechName.Refinery, ClientLib.Base.ETechName.PowerPlant], ClientLib.Base.EModifierType.CreditsPackageSize, ClientLib.Base.EModifierType.CreditsProduction, bOnlyTopBuildings, bOnlyAffordableBuildings);
								...
								if (currenttibpct>20){
									...
									if (typeof(mlist[0])=='object') {
										console.log(infolineHeader+infolineUnits+" - Skipped: "+infolineSkipped+" - Priority "+tprio+" Upg: "+mlist[0]['Type']+" lvl: "+mlist[0]['Level'])
...
//										console.debug(FlunikTools.Main.prototype.print_r(mlist));
								...
								if (lowestbuilding != null) {
									if (lowestbuilding.CanUpgrade() && currenttibpct>95) {
										//console.debug("FLUNIK: %d Default upgrade - lowest building is %d level %d",cityname, lowestbuildingname, lowestbuildinglevel);
										console.log(infolineHeader+infolineUnits+" - Skipped: "+infolineSkipped+" - Default Upg: "+lowestbuildingname+" lvl: "+lowestbuildinglevel)
										lowestbuilding.Upgrade();
										continue;
									}
								}
